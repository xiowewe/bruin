...
public class RedisJedisUtil {
    ...
    private static void initialPool() {
        //Redis服务器IP
        String HOST = "192.168.159.129"; //todo
        //Redis的端口号
        int PORT = NumberUtils.toInt("6379", 6379); //todo
        //访问密码
        String AUTH = "123456"; //todo

        try {
            JedisPoolConfig config = new JedisPoolConfig();
            //最大连接数，如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
            config.setMaxTotal(NumberUtils.toInt(null, 400)); //todo
            //最大空闲数，控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
            config.setMaxIdle(NumberUtils.toInt(null, 50)); //todo
            //最小空闲数
            config.setMinIdle(NumberUtils.toInt(null, 10)); //todo
...
//        return JavaJsonConvert.json2List(dataList.toString(), clazz); todo
    ...
    public static <T> String setObject(String key ,T obj){
        ...
        String result = null;
        try {
            byte[] data= ProtoStuffSerializerUtil.serialize(obj);
...
//                result= ProtoStuffSerializerUtil.deserialize(data, );
            }
        } catch (Exception e) {
    ...
    public synchronized static <T> T setList(String key, List<T> dataList){
        ...
        try {
            ...
            if(CollectionUtils.isNotEmpty(dataList)){
                byte[] data = ProtoStuffSerializerUtil.serializeList(dataList);
    ...
    public static <T> List<T> getList(String key){
        ...
        try {
            ...
            if(data != null && data.length > 0){
//                return ProtoStuffSerializerUtil.deserializeList(data); todo
                return null;
            }
        } catch (Exception e) {
